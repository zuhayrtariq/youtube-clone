...
export const suggestionsRouter = createTRPCRouter({
    getMany: baseProcedure.input(
        ...
        const [existingVideo] = await db.select().from(videosTable).where(eq(videosTable.id, videoId));
        if (!existingVideo) {
        ...
        const data = await db.select(
            ...
            .where(
                and(
                    ...
                    eq(
                        videosTable.visibility, 'public'
                    ),
                    existingVideo.categoryId ?
                        eq(videosTable.categoryId, existingVideo.categoryId) : undefined,
