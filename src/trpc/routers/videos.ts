...
export const videosRouter = createTRPCRouter({
  ...
  getManySubscriptions: protectedProcedure
    ...
    .query(async ({ input, ctx }) => {
      ...
      const data = await db
        ...
        .innerJoin(
          viewerSubscriptions,
          eq(viewerSubscriptions.userId, usersTable.id)
        )
        .where(
          and(
            eq(videosTable.visibility, "public"),
      ...
      const data = await db
        ...
        .where(
          and(
            ...
            cursor
              ? or(
                  lt(videosTable.views, cursor.views),
                  and(
                    eq(videosTable.views, cursor.views),
                    lt(videosTable.id, cursor.id)
                  )
                )
              : undefined
        ...
        .orderBy(desc(videosTable.views), desc(videosTable.id))
        .limit(limit + 1);
  ...
  revalidate: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      const [existingVideo] = await db
        ...
        .from(videosTable)
        .where(
          and(eq(videosTable.userId, userId), eq(videosTable.id, videoId))
        );
      if (!existingVideo) throw new TRPCError({ code: "NOT_FOUND" });

      const { muxUploadId } = existingVideo;
      if (!muxUploadId) throw new TRPCError({ code: "BAD_REQUEST" });
      const directUpload = await mux.video.uploads.retrieve(muxUploadId);
      const { asset_id } = directUpload;
      if (!asset_id) throw new TRPCError({ code: "BAD_REQUEST" });
      ...
      const updatedVideo = await db
        ...
        .where(and(eq(videosTable.userId, userId), eq(videosTable.id, videoId)))
        .returning();
      return updatedVideo;
    }),
  ...
  getOne: baseProcedure
    ...
    .query(async ({ input, ctx }) => {
      ...
      const [existingVideo] = await db
        ...
        .select({
          ...
          user: {
            ...
            subscriberCount: db.$count(
              subscribers,
              eq(subscribers.creatorId, videosTable.userId)
            ),
          },
          viewCount: db.$count(
            videoViews,
        ...
        .from(videosTable)
        .innerJoin(usersTable, eq(videosTable.userId, usersTable.id))
        .leftJoin(userReaction, eq(videosTable.id, userReaction.videoId))
  ...
  addView: baseProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      const { workflowRunId } = await workflow.trigger({
        url:
          process.env.UPSTASH_WORKFLOW_URL + "/api/videos/workflows/video-view",
        body: {
          videoId,
        },
      });
  ...
  generateThumbnail: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const { workflowRunId } = await workflow.trigger({
        url:
          process.env.UPSTASH_WORKFLOW_URL + "/api/videos/workflows/thumbnail",
        body: {
          userId,
          videoId: input.id,
          prompt: input.prompt,
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (existingVideo.thumbnailKey) {
        ...
        await db
          ...
          .where(and(eq(videosTable.userId, userId), eq(videosTable.id, id)));
      }
